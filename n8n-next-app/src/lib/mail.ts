...
export const sendResetPasswordEmail = async (email: string, code: string) => {
  const mailOptions = {
    from: process.env.GOOGLE_MAIL,
    to: email,
    subject: 'Reset Password Code for FlowX',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        <h2 style="color: #333; text-align: center;">FlowX Password Reset</h2>
        <p style="font-size: 16px; color: #555;">You requested a password reset. Your reset code is:</p>
